...
step 10 'Starting services'; systemctl enable lintech-api lintech-worker lintech-agent; systemctl restart lintech-agent; systemctl restart lintech-api; systemctl restart lintech-worker; systemctl reload nginx; for service in lintech-api lintech-worker lintech-agent nginx; do systemctl is-active --quiet "$service" || { systemctl --no-pager --full status "$service" || true; exit 1; }; done
