...
async function applicationsView() {
  ...
  const createForm = kind => {
    const runtime = kind === 'php' ? selectField('PHP version', 'phpVersion', [{ value: '8.3', label: 'PHP 8.3' }]) : kind === 'python' ? selectField('Python version', 'pythonVersion', [{ value: '3.12', label: 'Python 3.12' }]) : selectField('Node.js version', 'nodeVersion', [{ value: '18', label: 'Node.js 18' }]);
    const specialization = kind === 'php' ? selectField('Framework', 'framework', [{ value: 'generic', label: 'Generic PHP' }, { value: 'laravel', label: 'Laravel' }, { value: 'codeigniter', label: 'CodeIgniter' }]) : kind === 'python' ? selectField('Framework', 'framework', [{ value: 'flask', label: 'Flask' }, { value: 'django', label: 'Django' }, { value: 'generic_wsgi', label: 'Generic WSGI' }]) : kind === 'node' ? field('Entrypoint', 'entrypoint', 'text', 'server.js', { required: true, pattern: '[A-Za-z0-9_][A-Za-z0-9_./-]*\\.(js|mjs|cjs)' }) : field('Build output', 'outputDir', 'text', 'dist', { required: true, pattern: '[A-Za-z0-9_][A-Za-z0-9_./-]*' });
    const label = kind === 'php' ? 'PHP' : kind === 'python' ? 'Python' : kind === 'node' ? 'Node.js' : 'React';
    const form = node('form', { class: 'form-grid' }, selectField('Domain', 'domainId', available.map(item => ({ value: item.id, label: item.name }))), node('input', { type: 'hidden', name: 'kind', value: kind }), runtime, specialization, node('button', { type: 'submit', disabled: available.length ? null : true }, `Create ${label} application`));
    form.addEventListener('submit', async event => { event.preventDefault(); try { await api('/applications', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(form))) }); showNotice('Application provisioning queued.'); await applicationsView(); } catch (error) { showNotice(error.message, true); } });
    return form;
  };
  const control = async (item, action) => { try { await api(`/applications/${item.id}/${action}`, { method: 'POST', body: '{}' }); showNotice(`${action} queued.`); await applicationsView(); } catch (error) { showNotice(error.message, true); } };
  const remove = async item => { if (!confirm(`Remove the application from ${item.name}? Project and website files will be preserved.`)) return; try { await api(`/applications/${item.id}`, { method: 'DELETE', body: '{}' }); showNotice('Application removal queued.'); await applicationsView(); } catch (error) { showNotice(error.message, true); } };
  const rows = applications.map(item => {
    const busy = ['queued', 'deleting'].includes(item.status);
    const processActions = ['python', 'node'].includes(item.kind) ? [node('button', { class: 'ghost', type: 'button', disabled: busy ? true : null, onclick: () => control(item, item.status === 'stopped' ? 'start' : 'stop') }, item.status === 'stopped' ? 'Start' : 'Stop'), node('button', { class: 'ghost', type: 'button', disabled: busy ? true : null, onclick: () => control(item, 'restart') }, 'Restart')] : [];
    return node('tr', {}, node('td', { class: 'code' }, item.name), node('td', {}, item.runtime), node('td', {}, item.framework || item.entrypoint || '—'), node('td', {}, badge(item.status, item.status === 'active')), node('td', {}, date(item.createdAt)), node('td', { class: 'actions' }, ...processActions, node('button', { class: 'danger', type: 'button', disabled: busy ? true : null, onclick: () => remove(item) }, 'Remove')));
  });
  const table = applications.length ? node('div', { class: 'table-wrap' }, node('table', {}, node('thead', {}, node('tr', {}, ['Application', 'Runtime', 'Framework', 'Status', 'Created', 'Actions'].map(label => node('th', {}, label)))), node('tbody', {}, rows))) : empty('No applications yet.');
  const forms = available.length ? node('div', { class: 'two-column' }, panel('Create PHP application', createForm('php')), panel('Create Python application', createForm('python')), panel('Create Node.js application', createForm('node')), panel('Build React application', createForm('react'))) : empty('Create and activate an unused customer domain first.');
  content.replaceChildren(forms, node('br'), panel('Applications', table));
}
