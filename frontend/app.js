...
async function applicationsView() {
  ...
  const createForm = kind => {
    const runtime = kind === 'php' ? selectField('PHP version', 'phpVersion', [{ value: '8.3', label: 'PHP 8.3' }]) : selectField('Python version', 'pythonVersion', [{ value: '3.12', label: 'Python 3.12' }]);
    const frameworks = kind === 'php' ? [{ value: 'generic', label: 'Generic PHP' }, { value: 'laravel', label: 'Laravel' }, { value: 'codeigniter', label: 'CodeIgniter' }] : [{ value: 'flask', label: 'Flask' }, { value: 'django', label: 'Django' }, { value: 'generic_wsgi', label: 'Generic WSGI' }];
    const form = node('form', { class: 'form-grid' }, selectField('Domain', 'domainId', available.map(item => ({ value: item.id, label: item.name }))), node('input', { type: 'hidden', name: 'kind', value: kind }), runtime, selectField('Framework', 'framework', frameworks), node('button', { type: 'submit', disabled: available.length ? null : true }, `Create ${kind === 'php' ? 'PHP' : 'Python'} application`));
  ...
  const rows = applications.map(item => {
    const busy = ['queued', 'deleting'].includes(item.status);
    const processActions = item.kind === 'python' ? [node('button', { class: 'ghost', type: 'button', disabled: busy ? true : null, onclick: () => control(item, item.status === 'stopped' ? 'start' : 'stop') }, item.status === 'stopped' ? 'Start' : 'Stop'), node('button', { class: 'ghost', type: 'button', disabled: busy ? true : null, onclick: () => control(item, 'restart') }, 'Restart')] : [];
    return node('tr', {}, node('td', { class: 'code' }, item.name), node('td', {}, item.runtime), node('td', {}, item.framework), node('td', {}, badge(item.status, item.status === 'active')), node('td', {}, date(item.createdAt)), node('td', { class: 'actions' }, ...processActions, node('button', { class: 'danger', type: 'button', disabled: busy ? true : null, onclick: () => remove(item) }, 'Remove')));
  });
  ...
  const forms = available.length ? node('div', { class: 'two-column' }, panel('Create PHP application', createForm('php')), panel('Create Python application', createForm('python'))) : empty('Create and activate an unused customer domain first.');
