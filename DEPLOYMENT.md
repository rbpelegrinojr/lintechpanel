...
The supported target is a fresh Ubuntu Server 24.04 LTS x86_64 VPS. Version 0.1.0 may be installed for evaluation using `installer/install.sh`; it is not approved for public multi-tenant production. Public ports are 22/TCP, 80/TCP, and 443/TCP. PostgreSQL, MariaDB, the API port 8080, worker, and agent socket must remain private.
