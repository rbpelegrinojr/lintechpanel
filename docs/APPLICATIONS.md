...
Node.js and React remain architectural targets. Never treat a queued job as deployed: only a `successful` job and `active` application record indicate that the agent completed its validation path, and the current Linux provisioners still require external Ubuntu testing.
