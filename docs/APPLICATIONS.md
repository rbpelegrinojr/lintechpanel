...
React publishing supports a generated React 19/Vite 6 starter or an existing package, tenant-owned dependency installation, a fixed build script, configurable relative output directory, rejection of symlinks/special files/oversized trees, and atomic publication with rollback. Static domains already map to isolated document roots. Upload/Git source acquisition and live Ubuntu build validation remain outstanding.

Never treat a queued job as deployed: only a `successful` job and `active` application record indicate that the agent completed its validation path, and the current Linux provisioners still require external Ubuntu testing.
